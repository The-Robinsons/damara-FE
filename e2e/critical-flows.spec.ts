...
test("공구 등록은 선택한 다마라존을 pickupZoneId로 전송한다", async ({ page }) => {
      ...
      body: JSON.stringify({
        items: [
          ...
          },
          {
            ...
            campusLabel: "인문캠퍼스",
            description: "인문캠퍼스 기숙사 1층 로비 공식 접선지",
            isActive: true,
            sortOrder: 20,
          },
        ],
  ...
  await page.getByLabel("모집 인원 (모집자 미포함)").fill("3");
  await page.getByRole("button", { name: "다음", exact: true }).click();
  await expect(page.getByText("인문캠퍼스", { exact: true })).toBeVisible();
  await expect(page.getByText("인문캠퍼스 기숙사 1층 로비 공식 접선지")).toHaveCount(0);
...
test("참여자가 없는 모집글은 거래 완료 요청을 보낼 수 없다", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "거래 완료하기" })).toBeDisabled();
  await expect(page.getByText("참여자가 한 명 이상 있어야 거래를 완료할 수 있어요.")).toBeVisible();
