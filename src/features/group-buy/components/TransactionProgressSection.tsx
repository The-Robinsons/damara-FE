...
export default function TransactionProgressSection({
  ...
  const actualParticipants = participants.filter((person) => person.userId !== currentUserId);
  const requiresParticipant = nextPostStatus === "closed" || nextPostStatus === "completed";
  const canChangePostStatus = !requiresParticipant || actualParticipants.length > 0;
  const recruitmentBlockedMessage = "모집 마감을 하시려면 참여자가 한 명 이상 필요합니다.";
  const completionBlockedMessage = "참여자가 한 명 이상 있어야 거래를 완료할 수 있어요.";
  const postStatusBlockedMessage = nextPostStatus === "closed" ? recruitmentBlockedMessage : completionBlockedMessage;
  ...
  const changePostStatus = async () => {
    ...
    if (!canChangePostStatus) {
      setError(postStatusBlockedMessage);
      return;
    }
    try {
      setBusyKey("post");
      ...
      <div style={{ marginTop: 12, padding: "12px", borderRadius: 8, border: `1px solid ${grey200}`, background: "#fff" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          ...
          {isOwner && nextPostStatus ? (
            <ActionButton size="compact" disabled={Boolean(busyKey) || (nextPostStatus === "completed" && !canChangePostStatus)} onClick={() => void changePostStatus()} style={{ height: 36 }}>
              {POST_ACTION_LABELS[nextPostStatus] ?? POST_STATUS_META[nextPostStatus].label}
              <ArrowRight size={14} aria-hidden />
            </ActionButton>
          ) : null}
        </div>
        {isOwner && !canChangePostStatus ? <span style={{ display: "block", marginTop: 8, color: grey500, fontSize: 11, fontWeight: 650 }}>{postStatusBlockedMessage}</span> : null}
